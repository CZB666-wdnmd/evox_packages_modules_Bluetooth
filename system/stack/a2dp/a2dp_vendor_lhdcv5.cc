...
static tA2DP_STATUS A2DP_ParseInfoLhdcV5(tA2DP_LHDCV5_CIE* p_ie,
  ...
  LOG_DEBUG( "%s: %s isCap{%d} SR{%02X} BPS{%02X} Ver{%02X} FL{%02X} "
      "MBR{%02X} mBR{%02X} Feature{AR(%d) JAS(%d) META(%d) LL(%d) LLESS(%d) LLESS24(%d) LLESS96K(%d) LLESSRaw(%d)}",
      __func__,
      (is_source?"SRC":"SNK"),
      is_capability,
      p_ie->sampleRate,
  ...
  if ((codec_config_.codec_specific_1 & A2DP_LHDC_VENDOR_CMD_MASK) ==
    ...
    switch (bitRateIndex) {
      ...
      case A2DP_LHDC_QUALITY_ABR:
        return 9999999;
      default:
        LOG_DEBUG("%s: non-supported bitrate index (%d)", __func__, bitRateIndex);
        return -1;
    }
  }
  return 400000;
...
bool A2dpCodecConfigLhdcV5Base::setCodecConfig(const uint8_t* p_peer_codec_info,
  ...
  LOG_DEBUG( "%s: sampleRate:{peer:0x%02X local:0x%02X cap:0x%02X user:0x%02X}",
      __func__, sink_info_cie.sampleRate, p_a2dp_lhdcv5_caps->sampleRate,
  ...
  do {
    ...
    if (codec_config_.sample_rate != BTAV_A2DP_CODEC_SAMPLE_RATE_NONE) {
      LOG_DEBUG( "%s: sample rate configured by UI successfully 0x%02X",
  ...
  LOG_DEBUG( "%s: bitsPerSample:{peer:0x%02X local:0x%02X cap:0x%02X user:0x%02X}",
      __func__, sink_info_cie.bitsPerSample, p_a2dp_lhdcv5_caps->bitsPerSample,
  ...
  do {
    ...
    if (codec_config_.bits_per_sample != BTAV_A2DP_CODEC_BITS_PER_SAMPLE_NONE) {
      LOG_DEBUG( "%s: bit_per_sample configured by UI successfully 0x%02X",
  ...
  if ((codec_user_config_.codec_specific_3 & A2DP_LHDC_VENDOR_FEATURE_MASK) != A2DP_LHDCV5_FEATURE_MAGIC_NUM)
  {
    ...
    LOG_DEBUG( "%s: LHDC features tag check fail, default UI status[LL] => %s", __func__, hasUserSet?"true":"false");
    ...
    A2DP_UpdateFeatureToA2dpConfigLhdcV5(
        ...
        A2DP_LHDC_TO_A2DP_CODEC_USER_,
        (hasUserSet?true:false));
    LOG_DEBUG( "%s: LHDC features tag check fail, default UI status[AR] => %s", __func__, hasUserSet?"true":"false");

    // Feature: Lossless (default UI: ON)
    hasUserSet = true;
    A2DP_UpdateFeatureToA2dpConfigLhdcV5(
      &allCfgPack,
      LHDCV5_FEATURE_CODE_LLESS,
      A2DP_LHDC_TO_A2DP_CODEC_USER_,
      (hasUserSet?true:false));
    LOG_DEBUG( "%s: LHDC features tag check fail, default UI status[LLESS] => %s", __func__, hasUserSet?"true":"false");
    ...
    A2DP_UpdateFeatureToA2dpConfigLhdcV5(
      ...
      A2DP_LHDC_TO_A2DP_CODEC_USER_,
      (hasUserSet?true:false));
    LOG_DEBUG( "%s: LHDC features tag check fail, default UI status[LLESS Raw] => %s", __func__, hasUserSet?"true":"false");
#endif
  }
   ...
   *******************************************/
  {
    ...
    LOG_DEBUG( "%s: featureJAS: enabled? <%s> {Peer:0x%02X Local:0x%02X}", __func__,
   ...
   *******************************************/
  {
    ...
    LOG_DEBUG( "%s: featureAR: enabled? <%s> {Peer:0x%02X Local:0x%02X User:%s}", __func__,
   ...
   *******************************************/
  {
    ...
    LOG_DEBUG( "%s: featureMETA: enabled? <%s> {Peer:0x%02X Local:0x%02X}", __func__,
   ...
   *******************************************/
  {
    ...
    LOG_DEBUG( "%s: featureLLESS: enabled? <%s> {Peer:0x%02X Local:0x%02X User:%s 24bit:%s 96K:%s}", __func__,
   ...
   *******************************************/
  {
    ...
    LOG_DEBUG( "%s: featureLLESSRaw: enabled? <%s> {Peer:0x%02X Local:0x%02X User:%s}", __func__,
  ...
  if(result_config_cie.hasFeatureLLESS48K == true) {
    ...
    if (bitRateQmode != A2DP_LHDC_QUALITY_HIGH1) {
      ...
      result_config_cie.hasFeatureLLESSRaw = false;
#endif
      LOG_DEBUG( "%s: disable lossless due to peer Max Bit Rate not NO LIMIT", __func__);
    }
  ...
  if (result_config_cie.hasFeatureLLESS48K == true) {
    ...
    if (result_config_cie.hasFeatureLLESSRaw == true) {
      ...
      if (result_config_cie.hasFeatureLLESS96K == false) {
        ...
        codec_user_config_.sample_rate = BTAV_A2DP_CODEC_SAMPLE_RATE_48000;
        result_config_cie.sampleRate = A2DP_LHDCV5_SAMPLING_FREQ_48000;
        LOG_DEBUG( "%s: (lossless raw) set 48KHz sample rate", __func__);
      } else {
        ...
        if (codec_user_config_.sample_rate != BTAV_A2DP_CODEC_SAMPLE_RATE_48000 &&
          ...
          codec_user_config_.sample_rate = BTAV_A2DP_CODEC_SAMPLE_RATE_48000;
          result_config_cie.sampleRate = A2DP_LHDCV5_SAMPLING_FREQ_48000;
          LOG_DEBUG( "%s: (lossless raw) default 48KHz sample rate", __func__);
        } else {
          //  48 or 96KHz => use sample rate from user
          LOG_DEBUG( "%s: (lossless raw) apply %s sample rate", __func__,
              lhdcV5_sampleRate_toString(result_config_cie.sampleRate).c_str());
        }
      }
    } else {
      ...
      if (result_config_cie.hasFeatureLLESS96K == false) {
        ...
        codec_user_config_.sample_rate = BTAV_A2DP_CODEC_SAMPLE_RATE_48000;
        result_config_cie.sampleRate = A2DP_LHDCV5_SAMPLING_FREQ_48000;
        LOG_DEBUG( "%s: (lossless) fixed 48KHz sample rate", __func__);
      } else {
        ...
        if (codec_user_config_.sample_rate != BTAV_A2DP_CODEC_SAMPLE_RATE_48000 &&
          ...
          codec_user_config_.sample_rate = BTAV_A2DP_CODEC_SAMPLE_RATE_48000;
          result_config_cie.sampleRate = A2DP_LHDCV5_SAMPLING_FREQ_48000;
          LOG_DEBUG( "%s: (lossless) default 48KHz sample rate", __func__);
        } else {
          // user configure 48/96KHz
          //  => use sample rate from user
          LOG_DEBUG( "%s: (lossless) apply %s sample rate", __func__,
              lhdcV5_sampleRate_toString(result_config_cie.sampleRate).c_str());
        }
      }
    }
  ...
  if (result_config_cie.hasFeatureAR == true) {
    ...
    if (codec_user_config_.bits_per_sample != BTAV_A2DP_CODEC_BITS_PER_SAMPLE_16 &&
      ...
      codec_user_config_.bits_per_sample = BTAV_A2DP_CODEC_BITS_PER_SAMPLE_24;
      result_config_cie.bitsPerSample = A2DP_LHDCV5_BIT_FMT_24;
      LOG_DEBUG( "%s: set 24 bits per sample for running AR", __func__);
    }
  }
    ...
    if (result_config_cie.hasFeatureLLESS24Bit == true) {
      if (codec_user_config_.bits_per_sample != BTAV_A2DP_CODEC_BITS_PER_SAMPLE_16 &&
        ...
        if (bitsPerSample & A2DP_LHDCV5_BIT_FMT_24) {
          ...
          codec_user_config_.bits_per_sample = BTAV_A2DP_CODEC_BITS_PER_SAMPLE_24;
          result_config_cie.bitsPerSample = A2DP_LHDCV5_BIT_FMT_24;
          LOG_DEBUG( "%s: (lossless-24bit): use default 24 mode", __func__);
        } else {
          ...
          result_config_cie.bitsPerSample = A2DP_LHDCV5_BIT_FMT_16;
          LOG_DEBUG( "%s: (lossless-24bit): use default 16 bit mode", __func__);
        }
      } else {
        // bitrate supported, adopt user's configuration
        if (codec_user_config_.bits_per_sample == BTAV_A2DP_CODEC_BITS_PER_SAMPLE_24) {
          LOG_DEBUG( "%s: (lossless-24bit): use 24 bit mode", __func__);
        } else {
          LOG_DEBUG( "%s: (lossless-24bit): use 16 bit mode", __func__);
        }
      }
    } else {
      ...
      codec_user_config_.bits_per_sample = BTAV_A2DP_CODEC_BITS_PER_SAMPLE_16;
      result_config_cie.bitsPerSample = A2DP_LHDCV5_BIT_FMT_16;
      LOG_DEBUG( "%s: (lossless-16bit): 16 bit mode only", __func__);
    }
  }
  ...
  if ((result_config_cie.hasFeatureLLESS48K == true) &&
    ...
    if (result_config_cie.hasFeatureLLESS24Bit == true) {
      if (result_config_cie.bitsPerSample == A2DP_LHDCV5_BIT_FMT_24) {
        LOG_DEBUG( "%s: (lossless 96KHz) : 24bit OK", __func__);
      } else {
        codec_config_.bits_per_sample = BTAV_A2DP_CODEC_BITS_PER_SAMPLE_24;
        codec_capability_.bits_per_sample = BTAV_A2DP_CODEC_BITS_PER_SAMPLE_24;
        codec_user_config_.bits_per_sample = BTAV_A2DP_CODEC_BITS_PER_SAMPLE_24;
        result_config_cie.bitsPerSample = A2DP_LHDCV5_BIT_FMT_24;
        LOG_DEBUG( "%s: (lossless 96KHz) : unsupported bit depth but 24bit capable, reconfigure to 24bit", __func__);
      }
    } else {
      codec_config_.sample_rate = BTAV_A2DP_CODEC_SAMPLE_RATE_48000;
      codec_capability_.sample_rate = BTAV_A2DP_CODEC_SAMPLE_RATE_48000;
      ...
      codec_user_config_.bits_per_sample = BTAV_A2DP_CODEC_BITS_PER_SAMPLE_16;
      result_config_cie.bitsPerSample = A2DP_LHDCV5_BIT_FMT_16;
      LOG_DEBUG( "%s: (lossless 96KHz) : 16 and 24bit not capable, reconfigure to 48KHz 16bit", __func__);
    }
  }
  ...
  if (result_config_cie.hasFeatureLLESS48K == true) {
    ...
    if ((qualityMode != A2DP_LHDC_QUALITY_ABR)) {
      LOG_DEBUG( "%s: reset non-supported quality mode (%s) to (%s) for lossless", __func__,
