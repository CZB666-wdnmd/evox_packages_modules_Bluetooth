...
bool A2dpCodecConfigLhdcV3Base::setCodecConfig(const uint8_t* p_peer_codec_info, bool is_capability,
  ...
  LOG_DEBUG( "%s: incoming version: peer(0x%02x), host(0x%02x)",
            __func__, sink_info_cie.version, a2dp_lhdcv3_sink_caps.version);
  ...
  LOG_DEBUG("%s: sampleRate:{peer:0x%02X local:0x%02X cap:0x%02X user:0x%02X}",
      __func__, sink_info_cie.sampleRate, a2dp_lhdcv3_sink_caps.sampleRate,
  ...
  if (codec_config_.sample_rate == BTAV_A2DP_CODEC_SAMPLE_RATE_NONE) {
    LOG_ERROR(
              ...
              "sink info = 0x%x",
              __func__, a2dp_lhdcv3_sink_caps.sampleRate, sink_info_cie.sampleRate);
    goto fail;
  }
  ...
  LOG_DEBUG("%s: bits_per_sample:{peer:0x%02X local:0x%02X cap:0x%02X user:0x%02X}",
      __func__, sink_info_cie.bits_per_sample, a2dp_lhdcv3_sink_caps.bits_per_sample,
  ...
  if (codec_config_.bits_per_sample == BTAV_A2DP_CODEC_BITS_PER_SAMPLE_NONE) {
    LOG_ERROR(
              ...
              __func__, a2dp_lhdcv3_sink_default_config.bits_per_sample,
  ...
  if (codec_config_.channel_mode == BTAV_A2DP_CODEC_CHANNEL_MODE_NONE) {
    LOG_ERROR("%s: codec_config_.channel_mode != BTAV_A2DP_CODEC_CHANNEL_MODE_NONE or BTAV_A2DP_CODEC_CHANNEL_MODE_STEREO"
              , __func__);
    goto fail;
  }
   ...
   *******************************************/
  {
    ...
    LOG_DEBUG( "%s: featureLLAC: enabled? <%s> {Peer:0x%02X Local:0x%02X}", __func__,
   ...
   *******************************************/
  {
    ...
    LOG_DEBUG( "%s: featureV4: enabled? <%s> {Peer:0x%02X Local:0x%02X}", __func__,
   ...
   *******************************************/
  {
    ...
    if (hasFeature && hasUserSet) {
      ...
      if(codec_user_config_.sample_rate > BTAV_A2DP_CODEC_SAMPLE_RATE_48000) {
        LOG_DEBUG( "%s: AR ON, adjust sample rate to 48KHz  ", __func__);
   ...
   *******************************************/
  {
    ...
    LOG_DEBUG( "%s: featureMBR: enabled? <%s> {Peer:0x%02X Local:0x%02X}", __func__,
  ...
  if (int ret = A2DP_BuildInfoLhdcV3Sink(AVDT_MEDIA_TYPE_AUDIO, &result_config_cie,
    ...
    LOG_ERROR("%s: A2DP_BuildInfoLhdcV3Sink fail(0x%x)", __func__, ret);
    goto fail;
  }
